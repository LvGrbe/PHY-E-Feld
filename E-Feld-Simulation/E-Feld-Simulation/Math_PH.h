...
namespace Physik 
{
    ...
    */
	...
	static double Länge_Vektor(sf::Vector2<double> a)
	{
	...
	static sf::Vector2<double> Elektrische_Feldstärke_Vektoren_Viele(sf::Vector2f punkt,std::vector<Punktladung>* Teilchen_vec)
	{
		...
		for (int i = 0; i < (*Teilchen_vec).size(); i++)
		{
			...
			Ergebniss += ((*Teilchen_vec)[i].Q / (4 * M_PI * Elektrische_Feldkonstante * pow(Physik::Länge_Vektor(OrtsVec), 3))) * OrtsVec;
		}
